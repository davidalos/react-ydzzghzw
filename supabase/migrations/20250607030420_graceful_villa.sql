/*
  ...
  1. Sample Data Creation
    - Creates sample clients (Íbúi 1-6)
    - Creates sample incidents with varied categories and dates
    - Creates sample goals with progress tracking
    - Creates sample goal updates (improvement, setback, neutral)

  2. Data Safety
    - Only creates data if it doesn't already exist
    - Uses real user IDs from existing user_profiles
    - Gracefully handles cases where no users exist yet
    - Provides informative notices about the seeding process

  3. Dashboard Population
    - Ensures Yfirlit page has data to display
    - Creates realistic incident distribution across categories
    - Provides goal progress data for charts and analytics
*/
...
BEGIN
  ...
  IF client_count < 6 THEN
    -- Insert each client individually to avoid unnest issues
